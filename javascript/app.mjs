...
import { initNotifications, spawnNotification } from './notifications.mjs';
...
class Strip {
  constructor(container) {
    ...
    initNotifications({
      parent: this.container.querySelector('nav'),
      getDB: this.getDB,
    });
  }
  ...
  async getDB() {
    ...
    this.dbPromise = new Promise((resolve, reject) => {
      const request = indexedDB.open('canvasDataDB', 2);
  ...
  async saveSession() {
    ...
    // - Intro text
    canvasData.objects = canvasData.objects.filter((obj) => !obj.noPersistence);
  ...
  async loadSession() {
    ...
    const objectStore = transaction.objectStore('canvasData');
    const request = objectStore.get('canvasData');
  ...
  async inpaint({ area = null, mode = 'default', alphaSrc = null }) {
    ...
    if (mode === 'blendWithBackground') {
      ...
      spawnNotification({
        icon: 'success',
        title: 'Use low denoise',
        showOnce: true,
        key: 'inpainting-parameters-set',
        subtitle:
          'A low denoising strength was set to keep img2img from changing too much detail from the image.',
        actions: [
          {
            name: 'Close',
          },
